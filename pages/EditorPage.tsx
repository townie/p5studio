...
import { useAutoSave, useUserProjects, useProject } from '@/hooks/useProject';
...
const EditorPage: React.FC = () => {
  ...
  // Load project from database when projectId is provided
  const { project, projectData, isLoading: projectLoading } = useProject(projectId);

  // Get user's recent projects for dropdown
  const { projects: recentProjects } = useUserProjects(user?.id);

  // Build project data for auto-save
  const currentProjectData: ProjectData = useMemo(() => ({
    name: projectName,
    history,
    currentIndex
  }), [projectName, history, currentIndex]);

  // Auto-save hook (only active when user is logged in and has a projectId)
  const { saveStatus, lastSaved, forceSave } = useAutoSave(
    projectId,
    user ? currentProjectData : null,
    user?.id
  );
  ...
  // Navigation handlers for dropdowns/menus
  const handleNavigate = (path: string) => {
    navigate(path);
  };

  const handleNewProject = () => {
    // Reset to a new project (navigate to editor without projectId)
    navigate('/editor');
    // Reset state
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-[#0a0a0a] text-zinc-100 selection:bg-zinc-800">
      ...
      <header className="h-14 border-b border-[#222] flex items-center justify-between px-6 bg-[#0d0d0d] z-20">
        {/* Left section: Logo and Projects dropdown */}
        <div className="flex items-center gap-4">
          ...
          {/* Projects dropdown - only show when logged in */}
          {user && (
            <>
        ...
        {/* Center section: Editor controls and view modes */}
        <div className="flex items-center gap-1">
          {/* Save Status - only show when logged in and has project */}
          {user && projectId && (
            <>
              <SaveStatus
                status={saveStatus}
          ...
          {/* Share Button - show when project exists */}
          {projectId && (
            <>
        ...
        <div className="flex items-center gap-2">
          {authLoading ? (
            ...
            <button
              ...
              className="px-4 py-1.5 text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 rounded-lg transition-all shadow-lg shadow-purple-900/20"
            >
              Sign In
            </button>
          )}
